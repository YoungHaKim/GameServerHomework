...
#ifndef _MSC_VER
const int Depth::kCountFieldNumber;
const int Depth::kQtyFieldNumber;
const int Depth::kPriceFieldNumber;
#endif  // !_MSC_VER
...
Depth* Depth::default_instance_ = NULL;

Depth* Depth::New() const {
  return new Depth;
}
  ...
  while ((tag = input->ReadTag()) != 0) {
    switch (::google::protobuf::internal::WireFormatLite::GetTagFieldNumber(tag)) {
      ...
      case 1: {
        ...
        if (input->ExpectTag(16)) goto parse_Qty;
        break;
      }
...
void Depth::CheckTypeAndMergeFrom(
    const ::google::protobuf::MessageLite& from) {
  MergeFrom(*::google::protobuf::down_cast<const Depth*>(&from));
}
...
#ifndef _MSC_VER
const int Feed::kProductCodeFieldNumber;
const int Feed::kBidDepthFieldNumber;
const int Feed::kAskDepthFieldNumber;
#endif  // !_MSC_VER
...
Feed* Feed::default_instance_ = NULL;
...
void Feed::CheckTypeAndMergeFrom(
    const ::google::protobuf::MessageLite& from) {
  MergeFrom(*::google::protobuf::down_cast<const Feed*>(&from));
}
...
bool Feed::IsInitialized() const {
  ...
  for (int i = 0; i < biddepth_size(); i++) {
    if (!this->biddepth(i).IsInitialized()) return false;
  }
  for (int i = 0; i < askdepth_size(); i++) {
    if (!this->askdepth(i).IsInitialized()) return false;
  }
  return true;
}
